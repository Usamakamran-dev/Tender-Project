...
function AdditionalDocumentForm() {
  ...
  const { translate , getHardcodedTranslation , selectedTender , setRefreshFlag } = useContext(TenderContext);
  ...
  const [loading, setLoading] = useState(false);
   ...
   useEffect(() => {
     const loadTranslations = async () => {
       const translations = {
         Additionaltitle: await translate('Upload Tender Document'),
  ...
  const handleSubmit = async () => {
    ...
    if (Object.keys(errors).length === 0) {
      try {
        ...
        formData.append('path_db', selectedTender);
  ...
  return (
    <div className="relative flex flex-col">
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        <h2 className="text-2xl font-bold mb-4">{translatedTexts.Additionaltitle}</h2>
